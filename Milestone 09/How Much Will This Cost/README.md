...
4. Set `MOCK_AI=true` only when running the deterministic local assignment validation; this bypasses OpenRouter and must not be described as real provider usage.
...
When an OpenRouter key is unavailable, run `MOCK_AI=true npm start` and send five authenticated requests using the notes in `test/`. The mock returns valid structured summaries and deterministic `prompt_tokens`, `completion_tokens`, and `total_tokens` values so the logging and cost-arithmetic workflow remains reproducible. The resulting evidence is explicitly local mock evidence, not five real OpenRouter calls.
