...
- **Response time:** `0.007644 seconds` (**7.6 ms**)
...
- **Content-Encoding:** none
- **Database queries:** 7 Prisma query log entries for the request on the supplied SQLite database. The count is constant for the page rather than growing with the number of orders because the route no longer performs per-order queries.
...
- **Response time:** `0.004090 seconds` (**4.1 ms**)
- **Content-Encoding:** `gzip` (verified in the response headers)
- **Size reduction from Phase 1:** **99.72%**
- **Size reduction from Phase 2:** **85.81%**
- **Database queries:** 7 Prisma query log entries, again independent of the number of returned orders.
